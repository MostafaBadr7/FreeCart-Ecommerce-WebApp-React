...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>[ Automation ]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    const handleClickGoogle = () => {
        ...
        const targetUrl = `https://accounts.google.com/o/oauth2/auth?redirect_uri=https://mostafabadr7.github.io/FreeCart-Ecommerce-WebApp-React/&response_type=token&client_id=${googleClientId}&scope=openid%20email%20profile`;
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>[ RETURN ]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  :
    <div className={`${style.loginMain} py-5 min-vh-100`}>
